...
toLua (L.List ((L.Symbol "if"):cond:(L.List body):rest)) = do
  ...
  case rest of
    ...
    m -> throwError $
         L.BadCall "either empty list or else condition and body for if call" m
...
toLua (L.List [L.Symbol "map", L.List args]) = (mapM toPairs args) >>= (\x -> return $ Map x)
  where toPairs (L.List pairs@[field, value]) = (mapM toLua pairs)
        toPairs m = throwError $ L.BadCall "pair of values in the form (field value) in map call" [m]
...
toLua (L.List (func:args)) = if (isUnsupported func) then
                               ...
                               (throwError $ L.NotFunc "Attempted to call something other than a function" func) 
                             else
