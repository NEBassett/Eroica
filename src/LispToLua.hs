...
toLua (L.List ((L.Symbol "if"):cond:(L.List body):rest)) = do
  ...
  case rest of
    ...
    m -> throwError $
         L.TypeMismatch "Expected either empty list or else condition and body for if call, got: " (L.List m)
...
toLua (L.List [L.Symbol "map", L.List args]) = (mapM toPairs args) >>= (\x -> return $ Map x)
  where toPairs (L.List pairs@[field, value]) = (mapM toLua pairs)
        toPairs m = throwError $ L.TypeMismatch "Expected pair of values in the form (field value) in map call, got: " m
